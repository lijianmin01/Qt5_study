...
class Demo(QWidget):
    ...
    def label_init(self):
        self.pic_label.setPixmap(QPixmap('../QIconImg/off.png'))

    def on_off_bulb_func(self):
        if self.off_button.isChecked():
            self.pic_label.setPixmap(QPixmap('../QIconImg/off.png'))
        else:
            self.pic_label.setPixmap(QPixmap('../QIconImg/on.png'))
...
我们可以看到在示例中先添加了一个大小为1的占位符，然后再添加用于显示图片的QLabel，最后再加了一个大小为1的占位符。
这样做的目的就是为了让这个QLabel控件居中。若将添加的第一个占位符(即左边的占位符)大小设为2，那么QLabel就会偏右，
因为左边的占位符大小比QLabel右边的占位符大；
