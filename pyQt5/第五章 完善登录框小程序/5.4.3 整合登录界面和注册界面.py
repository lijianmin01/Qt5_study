...
class Demo(QWidget):
    def __init__(self):
        ...
        self.forget_pwd = QPushButton('forget password',self)
        ...
        self.forget_pwd_page = ForgetPwd()
    ...
    def layout_init(self):
        ...
        self.grid_layout.addWidget(self.forget_pwd,2,0,1,2)
    ...
    def pushbutton_init(self):
        ...
        self.forget_pwd.clicked.connect(self.show_change_pwd_page_func)
    ...
    def show_change_pwd_page_func(self):
        self.forget_pwd_page.exec_()
...
class ForgetPwd(QDialog):
    def __init__(self):
        ...
        self.signin_pwd_line = QLineEdit(self)
        self.signin_pwd2_line = QLineEdit(self)
        self.change_pwd = QPushButton('Change Your password',self)
        ...
        # 对控件今次那个布局
        self.layout_init()
        # 在输入框中添加提示文字和判断是否都有输入
        self.lineedit_init()
        # 给按钮添加事件
        self.pushbutton_init()
    def layout_init(self):
        self.grid_layout.addWidget(self.signin_user_label,0,0,1,1)
        self.grid_layout.addWidget(self.signin_user_line,0,1,1,1)
        self.grid_layout.addWidget(self.signin_pwd_label,1,0,1,1)
        self.grid_layout.addWidget(self.signin_pwd_line,1,1,1,1)
        self.grid_layout.addWidget(self.signin_pwd2_label,2,0,1,1)
        self.grid_layout.addWidget(self.signin_pwd2_line,2,1,1,1)
        self.grid_layout.addWidget(self.change_pwd,3,1,1,1)

        self.all_layout.addLayout(self.grid_layout)

        self.setLayout(self.all_layout)

    def lineedit_init(self):
        self.signin_user_line.setPlaceholderText("Please enter your username")
        self.signin_pwd_line.setPlaceholderText("Please enter your new password")
        self.signin_pwd_line.setPlaceholderText("Please enter your new password again")
    ...
    def check_input_func(self):
        if self.signin_user_line.text() and self.signin_pwd_line.text() and self.signin_pwd2_line.text():
            self.change_pwd.setEnabled(True)
        else:
            self.change_pwd.setEnabled(False)

    def pushbutton_init(self):
        self.change_pwd.setEnabled(False)
        self.change_pwd.clicked.connect(self.check_change_pwd)

    def check_change_pwd(self):
        if self.signin_user_line.text() not in USER_PWD:
            QMessageBox.critical(self, 'Wrong', "Don't have this user!")
        else:
            if self.signin_pwd_line.text()!=self.signin_pwd2_line.text():
                QMessageBox.critical(self, 'Wrong', "两次输入的密码不一致")
            else:
                USER_PWD[self.signin_user_line.text()] = self.signin_pwd_line.text()
                QMessageBox.information(self, 'Information', '更改密码成功')
                self.close()
